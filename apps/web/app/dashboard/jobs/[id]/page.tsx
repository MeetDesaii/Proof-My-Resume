...
export default async function JobDetailPage({ params }: PageProps) {
  ...
  const parsedData = job.parsedData as {
    responsibilities?: string[];
    skills?: string[];
  } | null;
  const mustHaveReqs = job.requirements.filter(
    (r: any) => r.importance === "must_have"
  );
  const niceToHaveReqs = job.requirements.filter(
    (r: any) => r.importance === "nice_to_have"
  );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Added {formatDate(job.createdAt)} • {job.requirements.length}{" "}
                requirements
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Analyze how your resume matches this job description
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              {resumes.length === 0 ? (
                ...
                <div className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    {resumes.map((resume) => {
                      const hasMatches = resume.bullets.some(
                        (bullet: any) => bullet.matches.length > 0
                      );
                      return (
            ...
            <div className="space-y-6">
                      ...
                      >
                        ...
                        {req.matches.length > 0 && (
                          <p className="text-xs text-gray-600 mt-2">
                            {req.matches.length} resume bullets match this
                            requirement
                          </p>
                        )}
                      </div>
              ...
              {niceToHaveReqs.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {niceToHaveReqs.map((req: any) => (
                      <div
                        key={req.id}
                        className="border-l-2 border-l-green-500 pl-4 py-2"
                      >
                        <div className="flex items-start justify-between gap-4">
                          <p className="text-sm">{req.text}</p>
                          <Badge variant="outline">{req.category}</Badge>
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-2">
                  {parsedData.responsibilities.map(
